...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (empty($_POST["site_name"])) {
		echo ' <script type="text/javascript" charset="utf-8">alert("场地未选择");history.back();</script>';
		$checknum = 1;
		exit();
	} else {
		$site_ID = test_input($_POST["site_name"]);
	}
	...
	if (empty($_POST["num"])) {
		...
		$nowhave = "select nowhave from site where siteID = '" . $site_ID . "'";
	...
	if ($checknum == 0) {
		//插入数据
		$sql = "INSERT INTO booksite (bookID,siteID,borrower,number,borrowdate,returndate) VALUES ('','" . $site_ID . "','" . $stu_ID . "','" . $num . "','" . $borrow_date . "','" . $return_date . "')";
		//echo $sql."<br>";
		$result = mysql_query($sql);
		//echo $result;
		if ($result) {
			echo '<script type="text/javascript" charset="utf-8">alert("插入成功")</script>';
